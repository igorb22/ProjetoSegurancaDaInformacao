 ...
 * Adapted by abralvs on 13/03/20.
 */
...
public class SpinningWheelView extends View implements WheelRotation.RotationListener {
    ...
    private void drawWheelItems(Canvas canvas) {
        ...
        Typeface curr = this.textPaint.getTypeface();
        this.textPaint.setTypeface(Typeface.create(curr, Typeface.BOLD));
        textPaint.set(this.textPaint);
        float angle = getAnglePerItem() / 2;

        for (int i = 0; i < getItemSize(); i++) {
            CharSequence item = TextUtils
                    .ellipsize(items.get(i).toString().toUpperCase(), textPaint, textWidth, TextUtils.TruncateAt.END);
            canvas.save();
